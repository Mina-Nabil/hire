    ...
    public function updateCurrentBalance($employeeId, $key)
    {
        ...
        switch ($benefit['type']) {
            case VacationDetail::TYPE_YEARLY:
                ...
                $appliedVacations = AppliedVacation::getAppliedHours($startDate, $endOfYear, $benefit['name'], $benefit['id'] ?? null);
            ...
            case VacationDetail::TYPE_MONTHLY:
                ...
                $appliedVacations = AppliedVacation::getAppliedHours($startDate, $endOfMonth, $benefit['name'], $benefit['id'] ?? null);
                $this->employeesData[$employeeId]['vacationBenefits'][$key]['current_balance'] = ($value * $leftRatio) - $appliedVacations;
                break;
            case VacationDetail::TYPE_WEEKLY:
                $endOfWeek = $startDate->clone()->endOfWeek();
                $leftRatio = $endOfWeek->diffInDays($startDate, true) / 7;
                $appliedVacations = AppliedVacation::getAppliedHours($startDate, $endOfWeek, $benefit['name'], $benefit['id'] ?? null);
                $this->employeesData[$employeeId]['vacationBenefits'][$key]['current_balance'] = ($value * $leftRatio) - $appliedVacations;
                break;
            case VacationDetail::TYPE_QUARTERLY:
                ...
                $appliedVacations = AppliedVacation::getAppliedHours($startDate, $endOfQuarter, $benefit['name'], $benefit['id'] ?? null);
                $this->employeesData[$employeeId]['vacationBenefits'][$key]['current_balance'] = ($value * $leftRatio) - $appliedVacations;
                break;
