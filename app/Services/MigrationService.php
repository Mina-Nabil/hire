...
class MigrationService
{
    public static function migrateFromStartupfile($file)
    {
        ...
        DB::transaction(function () use ($template) {
            $departments_sheet = $template->getSheet(0);
            ...
            $employees_sheet = $template->getSheet(2);
            ...
            $positions_sheet = $template->getSheet(1);
            $highestRow = $positions_sheet->getHighestRow();
            for ($row = 2; $row <= $highestRow; $row++) {

                $departmentName = $positions_sheet->getCell('A' . $row)->getValue();
                $department = Department::where('name', $departmentName)->first();
                $positionName = $positions_sheet->getCell('B' . $row)->getValue();
                $positionArabicName = $positions_sheet->getCell('C' . $row)->getValue();
                $positionCode = $positions_sheet->getCell('D' . $row)->getValue();
                $parentPositionName = $positions_sheet->getCell('E' . $row)->getValue();
                $parentPosition = Position::where('name', $parentPositionName)->first();

                if (!$departmentName || !$positionName || !$positionArabicName) continue;

                if (!$department) {
                    throw new AppException('Department not found on row ' . $row);
                }
