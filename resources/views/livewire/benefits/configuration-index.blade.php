                    ...
                    <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                        <thead class="border-t border-slate-100 dark:border-slate-800 bg-slate-200 dark:bg-slate-700">
                            <tr>
                                ...
                                <th scope="col" class="table-th">Benefit Package</th>
                        ...
                        <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                            @foreach ($employees as $employee)
                                <tr class="even:bg-slate-100 dark:even:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-700">
                                    ...
                                    <td class="table-td">
                                        {{ $employee->benefitConfiguration && $employee->benefitConfiguration->package 
                                            ? $employee->benefitConfiguration->package->name 
                                            : 'N/A' }}
                                    </td>
                                    <td class="table-td">
                                        @if ($employee->benefitConfiguration)
                                            <span class="badge badge-{{ $employee->benefitConfiguration->status === 'active' ? 'success' : 'warning' }}">
                                                {{ ucfirst($employee->benefitConfiguration->status) }}
                                            </span>
                                        @else
                                    ...
                                    <td class="table-td">
                                        {{ $employee->benefitConfiguration 
                                            ? $employee->benefitConfiguration->created_at->format('Y-m-d') 
                                            : 'N/A' }}
                                    </td>
                                    <td class="table-td">
                                        <div class="flex space-x-3 rtl:space-x-reverse">
                                            <button class="action-btn btn-edit" wire:click="editConfiguration({{ $employee->id }})">
                    ...
                    <div class="card m-5 p-5">
                        <div class="card-body rounded-md bg-white dark:bg-slate-800">
                            <div class="items-center text-center p-5">
                                ...
                                <h2 class="card-title text-slate-900 dark:text-white mb-3">No configurations found with the
                                    applied filters</h2>
