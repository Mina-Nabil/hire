<div>
    <div class="flex justify-between items-center mb-6">
        <h4 class="font-medium lg:text-2xl text-xl capitalize text-slate-900 inline-block">Overtime Requests</h4>
    </div>
                    ...
                    <div class="overflow-hidden">
                        <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                            ...
                            <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                    ...
                                    <tr class="hover:bg-slate-200 dark:hover:bg-slate-700">
                                        ...
                                        <td class="table-td">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                @if($request->status === \App\Models\Attendance\Overtime::STATUS_PENDING)
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="table-td text-center">No overtime requests found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <x-modal wire:model="showDetailsModal" maxWidth="4xl">
                ...
                @if($selectedRequest)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div class="border rounded p-3">
                            ...
                            <p class="font-medium">{{ \Carbon\Carbon::parse($selectedRequest->date)->format('M d, Y') }}</p>
                        </div>
                        <div class="border rounded p-3">
                            <p class="text-sm text-slate-500">Time</p>
                            <p class="font-medium">{{ $selectedRequest->start_time }} - {{ $selectedRequest->end_time }}</p>
                        </div>
                        <div class="border rounded p-3">
                            <p class="text-sm text-slate-500">Hours</p>
                            <p class="font-medium">{{ $selectedRequest->hours }}</p>
                        </div>
                        <div class="border rounded p-3">
                            <p class="text-sm text-slate-500">Created By</p>
                            <p class="font-medium">{{ $selectedRequest->creator->name ?? 'N/A' }}</p>
                        </div>
                        <div class="border rounded p-3">
                        ...
                        <div class="border rounded p-3">
                            <p class="text-sm text-slate-500">Approved/Rejected Date</p>
                            <p class="font-medium">{{ \Carbon\Carbon::parse($selectedRequest->approved_at)->format('M d, Y h:i A') }}</p>
                        </div>
                        @endif
                    </div>

                    @if(isset($selectedRequest->admin_note) && $selectedRequest->admin_note)
                    ...
                    @if($selectedRequest->status === \App\Models\Attendance\Overtime::STATUS_APPROVED)
                    <div class="mt-4 bg-green-50 p-4 rounded border border-green-200">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-green-600">This overtime request has been approved and will be considered for compensation.</p>
                            </div>
                        </div>
                    </div>
                    @endif

                    @if($selectedRequest->status === \App\Models\Attendance\Overtime::STATUS_REJECTED)
                    <div class="mt-4 bg-red-50 p-4 rounded border border-red-200">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-red-600">This overtime request has been rejected and will not be considered for compensation.</p>
                            </div>
                        </div>
                    </div>
                    @endif
            ...
            <x-slot name="footer">
                <div class="flex justify-end space-x-3">
                    ...
                    @if($selectedRequest && $selectedRequest->status === \App\Models\Attendance\Overtime::STATUS_PENDING)
            ...
            <div class="py-4">
                @if($selectedRequest)
                    ...
                    <div class="mb-5">
                        ...
                        <textarea wire:model="adminNote" id="adminNote" class="form-control w-full" rows="3" placeholder="Add any additional notes about this overtime approval"></textarea>
                    ...
                    <div class="bg-blue-50 p-4 rounded border border-blue-200 mb-2">
                        <div class="flex items-start">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-blue-600">Approving this request will mark the overtime as approved and it will be considered for employee compensation.</p>
                            </div>
                        </div>
                    </div>
                @endif
            ...
            <div class="py-4">
                @if($selectedRequest)
                    ...
                    <div class="mb-5">
                        ...
                        <textarea wire:model="adminNote" id="adminNote" class="form-control w-full" rows="3" placeholder="Please provide a detailed reason for rejecting this overtime request"></textarea>
                        @error('adminNote') <span class="text-danger text-sm mt-1 block">{{ $message }}</span> @enderror
                    </div>
